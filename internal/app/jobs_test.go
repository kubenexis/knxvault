...
func TestJobRunnerStartNilLeader(t *testing.T) {
	runner := app.NewJobRunner(nil, nil, nil, nil, nil, config.Config{}, zap.NewNop())
...
func TestJobRunnerLeaseCleanupOnLeadership(t *testing.T) {
	...
	defer cancel()
	runner := app.NewJobRunner(&staticElector{leader: true}, dbSvc, nil, nil, leases, cfg, zap.NewNop())
	runner.Start(runCtx)
	<-runCtx.Done()
...
func TestJobRunnerNonLeaderSkipsJobs(t *testing.T) {
	...
	defer cancel()
	runner := app.NewJobRunner(&staticElector{leader: false}, nil, nil, nil, nil, cfg, zap.NewNop())
	runner.Start(runCtx)
	<-runCtx.Done()
}
