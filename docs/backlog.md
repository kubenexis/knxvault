...
**Current focus:** [Tier H K8s ecosystem](#tier-h--kubernetes-ecosystem-eso-cert-manager-sdks) (W40-01–W40-07) → [Tier 0](#tier-0--secrets-manager-checklist-priority-0) (W37-01–W37-09) → remaining **W36** tiers (B–E). **Tier A (W36-01–05), Tier F (W38-* except W38-14/15 stubs), Tier G (CSI), webhook (W38-07)** are **shipped**. **Helm, Terraform, AWS IAM** → [long-term](#long-term-future) (LT-*).
...
| **W38-14** | §7.1 | Raft peer transport TLS | security | M | W37-01 | **Stub:** `KNXVAULT_RAFT_MTLS_*` config vars added; Dragonboat mTLS wiring deferred. | Peers reject unauthenticated connections; 3-node cluster forms with mTLS certs. |
